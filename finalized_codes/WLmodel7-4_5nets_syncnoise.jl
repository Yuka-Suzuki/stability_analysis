...
function synchnoises(rho_eta_fluc,L,r_std,n_diffenv)
	noises = zeros(L);
...
function synchnoises2(rho_eta_fluc,L,r_std,n_diffenv,env_mat)
# patches that have the same mean environmental condition will fluctuate exactly the same way.
	node_id = zeros(Int16,size(env_mat));
	for i in 1:L
		if i > 1
			if env_mat[i] in env_mat[1:i-1]
				node_id[i] = findall(x->x==env_mat[i],env_mat)[1][1];
			else
				node_id[i] = -1;
			end;
		else
...
if c > 0
	c = rand(Uniform(0.005,0.015),(n_sp,n_sp)); 
	c = c .- (zeros(size(c)) + I).*diag(c); # interspecific interaction
end;
...
for rep in 1:nRep 
	for neu in [false]
		...
		for g in (1:maxtime)
			if g % 20 == 1
				env_noises = synchnoises2(rho_eta_fluc,length(env_mat),r_std,n_diffenv,env_mat);
